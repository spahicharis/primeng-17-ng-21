    ...
    @if (mode === "advanced") {
      ...
      >
        ...
        <div
          ...
          [attr.data-pc-section]="'buttonbar'"
        >
          @if (!headerTemplate) {
            <span
              ...
              [ngClass]="{
                'p-focus': focus,
                'p-disabled': disabled || isChooseDisabled(),
              }"
              (focus)="onFocus()"
              (blur)="onBlur()"
              pRipple
            ...
            >
              ...
              @if (chooseIcon) {
                <span
                  [ngClass]="'p-button-icon p-button-icon-left'"
                  [class]="chooseIcon"
              ...
              @if (!chooseIcon) {
                @if (!chooseIconTemplate) {
                  <PlusIcon
                    [styleClass]="'p-button-icon p-button-icon-left'"
                    [attr.aria-label]="true"
                    [attr.data-pc-section]="'chooseicon'"
                  />
                }
                @if (chooseIconTemplate) {
                  ...
                  >
                    <ng-template
                      *ngTemplateOutlet="chooseIconTemplate"
                    ></ng-template>
                  </span>
                }
              }
              <span
                ...
                [attr.data-pc-section]="'choosebuttonlabel'"
                >{{ chooseButtonLabel }}</span
              >
            </span>
              ...
              >
                ...
                @if (!uploadIcon) {
                  ...
                  @if (uploadIconTemplate) {
                    ...
                    >
                      <ng-template
                        *ngTemplateOutlet="uploadIconTemplate"
                      ></ng-template>
                    </span>
                  }
                }
              ...
              >
                ...
                @if (!cancelIcon) {
                  ...
                  @if (cancelIconTemplate) {
                    ...
                    >
                      <ng-template
                        *ngTemplateOutlet="cancelIconTemplate"
                      ></ng-template>
                    </span>
                  }
                }
      ...
      >
        ...
        <span
          [ngClass]="{
            ...
            'p-button-icon-only': !basicButtonLabel,
        ...
        >
          @if (hasFiles() && !auto) {
            ...
            @if (!chooseIcon) {
              @if (!chooseIconTemplate) {
                <PlusIcon
                  [styleClass]="'p-button-icon p-button-icon-left pi'"
                  [attr.aria-hidden]="true"
                  [attr.data-pc-section]="'uploadicon'"
                />
              }
              @if (chooseIconTemplate) {
                <span
          ...
          @if (basicButtonLabel) {
            <span class="p-button-label" [attr.data-pc-section]="'label'">{{
              basicButtonLabel
            }}</span>
          }
          @if (!hasFiles()) {
  ...
  implements AfterViewInit, AfterContentInit, OnInit, OnDestroy, BlockableUI
{
   ...
   * @group Emits
   */
  @Output() onBeforeUpload: EventEmitter<FileBeforeUploadEvent> =
    new EventEmitter<FileBeforeUploadEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onError: EventEmitter<FileUploadErrorEvent> =
    new EventEmitter<FileUploadErrorEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onProgress: EventEmitter<FileProgressEvent> =
    new EventEmitter<FileProgressEvent>();
  /**
   ...
   * @group Emits
   */
  @Output() onRemoveUploadedFile: EventEmitter<RemoveUploadedFileEvent> =
    new EventEmitter<RemoveUploadedFileEvent>();

  @ContentChildren(PrimeTemplate) templates:
    | QueryList<PrimeTemplate>
    | undefined;

  @ViewChild("advancedfileinput") advancedFileInput:
    | ElementRef
    | undefined
    | any;
  ...
  onFileSelect(event: any) {
    ...
    if (
      this.hasFiles() &&
      this.auto &&
      (this.mode !== "advanced" || !this.isFileLimitExceeded())
    ) {
      this.upload();
    }
  ...
  isIE11() {
    if (isPlatformBrowser(this.platformId)) {
      return (
        !!(this.document.defaultView as any)["MSInputMethodContext"] &&
        !!(this.document as any)["documentMode"]
      );
    }
  }
  ...
  isFileLimitExceeded() {
    ...
    const totalFileCount = isAutoMode
      ? this.files.length
  ...
  checkFileLimit(files: File[]) {
    this.msgs ??= [];
    const hasExistingValidationMessages =
      this.msgs.length > 0 && this.fileLimit < files.length;
    if (this.isFileLimitExceeded() || hasExistingValidationMessages) {
      this.msgs.push({
        severity: "error",
        summary: this.invalidFileLimitMessageSummary.replace(
          "{0}",
          (this.fileLimit as number).toString(),
        ),
  ...
  get chooseButtonLabel(): string {
    return (
      this.chooseLabel || this.config.getTranslation(TranslationKeys.CHOOSE)
    );
  }

  get uploadButtonLabel(): string {
    return (
      this.uploadLabel || this.config.getTranslation(TranslationKeys.UPLOAD)
    );
  }

  get cancelButtonLabel(): string {
    return (
      this.cancelLabel || this.config.getTranslation(TranslationKeys.CANCEL)
    );
  }

  get browseFilesLabel(): string {
    return this.config.getTranslation(TranslationKeys.ARIA)[
      TranslationKeys.BROWSE_FILES
    ];
  }
